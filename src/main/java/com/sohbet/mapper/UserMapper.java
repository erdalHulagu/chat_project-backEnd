...
//    UserDTO userToUserDto(User user);
    ...
    @Mapping(target = "roles", ignore = true)
//    @Mapping(target = "roles", source = "roles", qualifiedByName = "mapRolesToSetRole")
    ...
    default List<ChatDTO> mapChatsToChatDTOList(List<Chat> chats) {
    	return chats != null ? chats.stream().map(ChatMapper.CHATMAPPER::chatToChatDTO).collect(Collectors.toList()) : null;
    }

    @Named("mapChatsToChatDTOSet")
    default Set<ChatDTO> mapChatsToChatDTOSet(Set<Chat> chats) {
        return chats != null ? chats.stream().map(ChatMapper.CHATMAPPER::chatToChatDTO).collect(Collectors.toSet()) : null;
    }
